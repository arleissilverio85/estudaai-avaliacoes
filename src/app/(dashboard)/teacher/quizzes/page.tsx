  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-slate-900">Avaliações Criadas</h2>

        {quizzes.length === 0 ? (
          <div className="rounded-3xl border-2 border-dashed border-slate-200 bg-white p-12 text-center">
            <FileCheck className="mx-auto h-12 w-12 text-slate-300" />
            <h3 className="mt-3 text-base font-bold text-slate-900">Nenhuma avaliação cadastrada</h3>
                ...
                <Card key={q.id}>
                  <CardHeader className="pb-2">
                    <div className="flex items-start justify-between gap-2">
                      <CardTitle className="text-base font-bold text-slate-900">{q.title}</CardTitle>
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold border ${statusInfo.color}`}>
                        {statusInfo.label}
                      </span>
                    </div>
                    <CardDescription className="text-xs flex items-center gap-1 mt-1 text-indigo-600 font-medium">
                      <School className="h-3 w-3" />
                      {q.classrooms?.name || 'Sala Geral'}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 text-xs text-slate-500">
                    <p>Tipo: <span className="font-semibold text-slate-700">{q.question_type}</span></p>
